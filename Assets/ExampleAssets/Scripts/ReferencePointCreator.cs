...
public class ReferencePointCreator : MonoBehaviour
{
    ...
    private GameObject m_InstantiateGo;

    private ARSessionOrigin arOigin;
    ...
    void Awake()
    {
        ...
        arOigin = GetComponent<ARSessionOrigin>();
    ...
    void Update()
    {
        ...
        if (m_RaycastManager.Raycast(touch.position, s_Hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            if (m_InstantiateGo == null)
            {
                //m_InstantiateGo = Instantiate(m_ReferencePointPrefab, referencePoint.transform);
                m_InstantiateGo = Instantiate(m_ReferencePointPrefab);
            }

            m_InstantiateGo.transform.position = referencePoint.transform.position;
            Vector3 initv3 = m_InstantiateGo.transform.localEulerAngles;
            Vector3 camerav3 = arOigin.camera.transform.localEulerAngles;
            m_InstantiateGo.transform.localRotation = Quaternion.Euler(new Vector3(initv3.x, camerav3.y, initv3.z));

            if (referencePoint == null)
            {
                Debug.Log("shanks>>>>>Error creating reference point.");
            }
            else
            {
